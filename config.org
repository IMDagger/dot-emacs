...
* Define vars to enable/disable some custom UI behavior
#+begin_src emacs-lisp
(defvar cb:use-visual-bell? t
  "Set to nil when any type of bell is annoying fo you.")
(defvar cb:aggressive-kill? t
  "It tries to kill whole line if it's empty after previous kill.")
(defvar cb:default-font-size 15
  "I prefer large letters due to poor my eyesight.")
(defvar cb:ergonomic-command-executing? nil
  "Allow to define Shift+Space ergonimic command executor binding, it's disabled by default for me.")
(defvar cb:default-input-method "russian-computer"
  "Some people would prefer TeX input method.")
#+end_src
...
* Don't ring the bell on errors
  I like visual response when something is not ok.
#+begin_src emacs-lisp
(if cb:use-visual-bell?
  (setq visible-bell t)
...
#+end_src
* Input methods
** Use Russian (or TeX and etc) as the default alternative input method
This allows you to enter national letters.
#+begin_src emacs-lisp
(setq default-input-method cb:default-input-method)
#+end_src
** Define a key command for setting method
This allows you to enter mathematical symbols easily if you change to
...
#+begin_src emacs-lisp
(when cb:ergonomic-command-executing?
...
(bind-key* "<pause>" 'toggle-window-dedicated)
#+end_src
** Enable tabs subsystem, show tabs for buffers
#+begin_src emacs-lisp
...
(after 'tabbar
    (defun cb:tabbar-buffer-up ()
      (interactive)
      (other-window -1))
    (defun cb:tabbar-buffer-down ()
      (interactive)
      (other-window 1))
    (defun cb:tabbar-kill-buffer ()
      (interactive)
      (kill-buffer (current-buffer)))
    ...
    (bind-key* "S-<left>" 'tabbar-backward-tab)
    (bind-key* "S-<right>" 'tabbar-forward-tab)

    (bind-key* "<f10>" 'cb:tabbar-kill-buffer)
    (bind-key* "S-<up>" 'cb:tabbar-buffer-up)
    (bind-key* "S-<down>" 'cb:tabbar-buffer-down)
...
#+end_src
** Use Git only
Only enable Git and Bazaar by default, since all the projects I work on use Git and Bzr these days.
#+begin_src emacs-lisp
(setq vc-handled-backends '(Git Bzr))
#+end_src
** Autoload functions that are used throughout this config
#+begin_src emacs-lisp
(autoload 'vc-git-root "vc-git")
(autoload 'vc-bzr-root "vc-git")
#+end_src
* Enable commands
...
#+begin_src emacs-lisp
(set-frame-font (format "%s %s" (monospace-font) cb:default-font-size) t)
#+end_src
...
(defun cb-sp:kill-blank-lines (&optional arg)
  ...
  (cond
   ...
   (t
    ...
    (when (and cb:aggressive-kill? (is-current-blank?))
      (kill-append "\n" nil)
      (delete-char 1)))))
...
*** Key binding
#+begin_src emacs-lisp
(bind-key* "S-<f10>" 'printer-scanner-picker)
#+end_src
...
* SGML
  Enable sgml-mode for HTML-pages or templates.
#+begin_src emacs-lisp
...
(after 'sgml-mode
  (hook-fn 'html-mode-hook
    ...
    (cb:install-package 'web-mode t)
    (web-mode)))
(after 'web-mode
  (hook-fn 'web-mode-hook
...
#+begin_src emacs-lisp
(after 'scheme
  (cb:install-package 'rainbow-delimeters t))
#+end_src

Enable rainbow delimeters mode and key binding if it's necessary.
#+begin_src emacs-lisp
(after 'rainbow-delimeters
  (fn-hook 'rainbow-delimeters-hook
    (local-set-key (kbd "ESC ESC p") 'rainbow-delimiters-mode)
